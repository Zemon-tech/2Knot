    ...
    try {
      const { text } = await generateText({
        ...
        messages: [
          {
            role: "user",
            content: `Conversation transcript (truncated):\n\n${combined}`,
          },
        ],
  ...
  try {
    ...
    try {
      ...
      if (webSearch && env.SERPAPI_KEY) {
        try {
          ...
          try {
            res.write(
              `data: ${JSON.stringify({ type: "status", phase: "planning" })}\n\n`,
            );
          } catch {}
          ...
          try {
            ...
            if (Array.isArray(parsed))
              planned = parsed
                .map((p: any) => ({ query: String(p.query || "").trim() }))
                .filter((p) => p.query);
          } catch {
          ...
          if (planned.length === 0)
            planned = [{ query: message.slice(0, 300) }];

          // Intent: detect news vs general (must be declared before normalizeQuery uses them)
          const intentIsNews =
            /\b(news|latest|today|this week|breaking|headline|update|updates|trending|trend|live)\b/i.test(
              message,
            );
          ...
          const wantsWeek = /\b(this week|past week|last week)\b/i.test(
            message,
          );
          ...
          try {
            res.write(
              `data: ${JSON.stringify({ type: "status", phase: "searching" })}\n\n`,
            );
          } catch {}
          ...
          for (const p of planned.slice(0, 6)) {
            try {
              const batch = intentIsNews
                ? await serpGoogleNewsLightSearch(p.query, {
                    num,
                    gl,
                    hl,
                    tbs,
                ...
                : await serpGoogleLightSearch(p.query, {
                    num,
                    gl,
                    hl,
                    tbs,
          ...
          try {
            res.write(
              `data: ${JSON.stringify({ type: "status", phase: "fetching" })}\n\n`,
            );
          } catch {}
          ...
          try {
            ...
            articleTexts = await fetchTopArticlesText(
              urls,
              Math.min(6, urls.length),
            );
          } catch {}
          ...
          try {
            const briefLines = top
              ...
              .map((r, i) => {
                ...
                const extracted = (articleTexts[i] || "")
                  .replace(/\s+/g, " ")
                  .trim();
                const basis =
                  extracted && extracted.length > 120
                    ? extracted.slice(0, 600)
                    : (r.snippet || "")
                        .replace(/\s+/g, " ")
                        .trim()
                        .slice(0, 300);
          ...
          try {
            res.write(
              `data: ${JSON.stringify({ type: "status", phase: "summarizing" })}\n\n`,
            );
          } catch {}
        } catch {}
      }
      ...
      try {
        res.write(
          `data: ${JSON.stringify({ type: "status", phase: "answering" })}\n\n`,
        );
      } catch {}
    ...
    try {
      ...
      for await (const delta of response.textStream) {
        ...
        if (outgoing) {
          res.write(
            `data: ${JSON.stringify({ type: "delta", delta: outgoing })}\n\n`,
          );
        }
        cleanBuffer = cleaned;
      }
    ...
    if (webResults && webResults.length) {
      res.write(
        `data: ${JSON.stringify({ type: "sources", sources: webResults })}\n\n`,
      );
    }
    if (webSummary) {
      res.write(
        `data: ${JSON.stringify({ type: "webSummary", summary: webSummary })}\n\n`,
      );
    }
    // status: complete
    try {
      res.write(
        `data: ${JSON.stringify({ type: "status", phase: "complete" })}\n\n`,
      );
    } catch {}
...
export async function listOpenRouterModels(
  ...
  try {
    ...
    const free = list.filter((m) => {
      ...
      const zeroish = (v: any) =>
        v === 0 || v === "0" || v === "0.0" || v === undefined || v === null;
...
export async function listGroqModels(
  ...
  try {
    if (!env.GROQ_API_KEY)
      throw createError(400, "GROQ_API_KEY not configured");
