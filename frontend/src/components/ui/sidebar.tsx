...
function SidebarMenuActionsMenu({
  onRename,
  onDelete,
  className,
}: {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <SidebarMenuAction className={className} aria-label="Actions" showOnHover>
...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn("flex flex-row items-center justify-between gap-2 p-2", className)}
      {...props}
    />
  )
}
